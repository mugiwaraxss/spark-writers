        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="mb-6 flex items-center justify-between">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
                    <i class="fas fa-edit text-primary-500 mr-2" style="color: #3b82f6;"></i>
                    Edit Your Review
                </h1>
            ...
            <div class="overflow-hidden rounded-lg bg-white shadow-md transition-all duration-300 hover:shadow-lg dark:bg-gray-800">
                <div class="p-6">
                    ...
                    <form action="{{ route('client.reviews.update', $review) }}" method="POST" class="animate-fade-in" style="animation-delay: 0.2s">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="mb-6">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <div class="transition-transform hover:scale-110">
                                    <input type="radio" id="rating-{{ $i }}" name="rating" value="{{ $i }}" class="peer hidden" {{ old('rating', $review->rating) == $i ? 'checked' : '' }} required>
                                    <label for="rating-{{ $i }}" class="cursor-pointer text-4xl transition-all duration-300 {{ old('rating', $review->rating) >= $i ? 'text-yellow-400' : 'text-gray-300' }} peer-checked:text-yellow-400 hover:text-yellow-400" style="{{ old('rating', $review->rating) >= $i ? 'color: #facc15;' : 'color: #d1d5db;' }} font-size: 2.5rem;">★</label>
                                </div>
                                @endfor
                            </div>
                            @error('rating')
                        ...
                        <div class="flex justify-end space-x-3">
                            <a href="{{ route('client.orders.show', $order) }}" class="flex items-center rounded-lg bg-primary-100 px-5 py-2.5 text-center text-sm font-bold text-primary-700 shadow-sm transition-all duration-300 hover:bg-primary-200 hover:shadow-md focus:outline-none focus:ring-4 focus:ring-primary-100 dark:bg-primary-800 dark:text-primary-100 dark:hover:bg-primary-700 dark:focus:ring-primary-700 !important" style="background-color: #dbeafe; color: #1d4ed8;">
